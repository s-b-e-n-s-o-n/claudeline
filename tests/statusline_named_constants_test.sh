...
assert_contains "$statusline" 'ALLTIME_NORMAL_FIXED_ITEM_COUNT=5' "statusline should name the fixed all-time normal item count"
...
assert_contains "$usage_lib" 'local days_until_x10k=$(( seconds_until_reset * 10000 / SECONDS_PER_DAY ))' "usage lib should use the named day duration"
...
assert_not_contains "$usage_lib" 'local days_until_x10k=$(( seconds_until_reset * 10000 / 86400 ))' "usage lib should not inline the day duration"
