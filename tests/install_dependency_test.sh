...
mkdir -p "$home_dir" "$shim_dir" "$download_dir/lib"
...
"$bash_bin" "$repo_root/install.sh" > "$tmpdir/install.out"

[ -f "$HOME/.claude/settings.json" ] || {
    echo "FAIL: installer should succeed without bc and create settings.json" >&2
    exit 1
}

assert_file_contains '"command": "~/.claude/statusline.sh"' "$HOME/.claude/settings.json" "installer writes statusLine command without bc installed"
